...
export function HelpPage() {
 ...
 <h1 className="font-heading text-h1 text-primary flex items-center gap-2 mb-4">
 <LifeBuoy size={32} />
 Help & Support
 </h1>
 ...
 <li className="flex items-center gap-2"><Mail size={16} className="text-primary" /> Email: <a href={`mailto:${COMPANY.SUPPORT_EMAIL}`} className="text-primary hover:underline">{COMPANY.SUPPORT_EMAIL}</a></li>
 <li className="flex items-center gap-2"><Phone size={16} className="text-primary" /> Phone: <a href={`tel:${COMPANY.SUPPORT_PHONE.replace(/\s/g, "")}`} className="text-primary hover:underline">{COMPANY.SUPPORT_PHONE}</a></li>
 </ul>
 </section>
 </div>
 </div>
