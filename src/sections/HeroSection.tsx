...
export function HeroSection() {
  ...
  return (
    <section className="relative min-h-[90vh] flex flex-col items-center justify-center text-center overflow-hidden bg-gradient-to-br from-primary via-surface to-primary">
      ...
      <div className="absolute top-8 left-4 md:left-12 animate-float text-center opacity-80">
        <Bot size={40} className="text-accent mx-auto" />
        <p className="mt-1 text-caption text-text-secondary font-semibold">AI-Powered</p>
      </div>

      <div className="absolute bottom-8 left-4 md:left-12 animate-float text-center opacity-80" style={{ animationDelay: "2s" }}>
        <BarChart3 size={40} className="text-primary-light mx-auto" />
        <p className="mt-1 text-caption text-text-secondary font-semibold">Data-Driven</p>
      </div>

      <div className="absolute bottom-8 right-4 md:right-12 animate-float text-center opacity-80" style={{ animationDelay: "4s" }}>
        <ArrowUpRight size={40} className="text-accent mx-auto" />
        <p className="mt-1 text-caption text-text-secondary font-semibold">Business Growth</p>
      </div>

      <div className="relative z-10 mt-10 max-w-5xl mx-auto px-4">
        <div className="mb-8">
          <h1 className="font-heading text-h1 md:text-[56px] font-bold text-text-primary drop-shadow-md leading-tight">
            Custospark empowers{" "}
            <span className="text-accent">{phrases[phraseIndex]}</span>
          </h1>
          <p className="mt-6 text-body-lg md:text-xl max-w-2xl mx-auto text-text-secondary">
